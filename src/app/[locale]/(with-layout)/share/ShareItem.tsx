...
type ShareItemProps = Pick<ShareItemModel, "id" | "title" | "photoUrls" | "legalDong"> & {
  date: string;
};

export default function ShareItem({ id, title, photoUrls, date, legalDong }: ShareItemProps) {
  return (
    <Link className="flex flex-row sm:flex-col cursor-pointer group" href={`/share/${id.toString()}`}>
      ...
      <div className="flex gap-1 flex-col p-1 overflow-hidden">
        ...
        <div className="flex gap-1 text-sm text-muted-foreground">
          <span>{legalDong.eupMyeonDongName}</span>
          <span>·</span>
          <time>{date}</time>
        </div>
      </div>
